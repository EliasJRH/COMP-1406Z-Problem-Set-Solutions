...
public class LinkedList {
    ...
    public boolean isInIncreasingOrder() {
        return isInIncreasingOrder(head, (byte) 0);
    }

    public boolean isInIncreasingOrder(Item curNode, byte lastData){
        if (head == null || head.next == null) return true; // returns true if the list is empty or has size of 1
        if (curNode.data < lastData) return false; //returns false if the current node's data is less than the last nodes data
        if (curNode.next == null) return true; //returns true is the next node is null, the reasoning is that if program has gotten
                                               // to the end of the list, then it hasn't found a lower number and therefore the list is increasing
        return isInIncreasingOrder(curNode.next, curNode.data);
        //if none of the above are true, the function is called with the next node, and the current nodes data
    }
}
