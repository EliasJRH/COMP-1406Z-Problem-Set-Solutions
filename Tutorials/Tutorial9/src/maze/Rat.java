...
public class Rat {
    ...
    // Move the maze.Rat to the given position
    ...
    public int freeSpaces(Maze m){
        frees = 0; //free spaces set to 0
        return freeSpacesHelper(m);
    }

    public int freeSpacesHelper(Maze m){
        if (m.wallAt(row, col) || m.hasBeenVisited(row, col))
            //If the spot is a wall or has been visited
            return 0; //return 0
        else {
            //Otherwise, mark it and increase the number of free spaces
            m.markVisited(row, col);
            frees++;
        }
        ...
        moveTo(row-1, col);
        freeSpacesHelper(m);
        moveTo(row+1, col); //Move back down
        moveTo(row+1, col); //Move down one
        freeSpacesHelper(m);
        moveTo(row-1,col); //Move back up
        moveTo(row, col-1); //Move left
        freeSpacesHelper(m);
        moveTo(row, col+1); //Move back right
        moveTo(row, col+1); //Move right again
        freeSpacesHelper(m);
        moveTo(row, col-1); //return to original position
        return frees; //return number of frees
    }
