...
public class DVDCollectionApp extends Application {
    ...
    private static final byte UNDO_LIST_CAPACITY = 5;
    private static final byte ADD_DVD = 0;
    private static final byte REMOVE_DVD = 1;

    private ArrayDeque<Pair<Byte,DVD>> undoList;
    private ArrayDeque<Pair<Byte,DVD>> redoList;

    public DVDCollectionApp() {
        model = DVDCollection.example2();
        undoList = new ArrayDeque<Pair<Byte,DVD>>();
        redoList = new ArrayDeque<Pair<Byte,DVD>>();
    }

    private void performOperation(byte b, DVD obj){
        if (undoList.size() == UNDO_LIST_CAPACITY) {
            undoList.removeFirst();
        }
        undoList.addLast(new Pair(b,obj));
    ...
    public void start(Stage primaryStage) {
        ...
        menuBar.getMenus().addAll(fileMenu, editMenu, sortMenu);
        ...
        undoItem.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent actionEvent) {
                if (!undoList.isEmpty()){
                    if (redoList.size() == UNDO_LIST_CAPACITY){
                        redoList.removeFirst();
                    }
                    redoList.add(undoList.getLast());
                    byte operation = undoList.getLast().getKey();
                    if (operation == ADD_DVD){
                        model.remove(undoList.getLast().getValue().getTitle());
                    }else{
                        String title = undoList.getLast().getValue().getTitle();
                        int year = undoList.getLast().getValue().getYear();
                        int duration = undoList.getLast().getValue().getDuration();
                        DVD dvdToAdd = new DVD(title, year, duration);
                        model.add(dvdToAdd);
                    }
                    update(model, -1);
                    undoList.removeLast();
        ...
        redoItem.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent actionEvent) {
                byte operation = redoList.getLast().getKey();
                if (operation == ADD_DVD){
                    String title = redoList.getLast().getValue().getTitle();
                    int year = redoList.getLast().getValue().getYear();
                    int duration = redoList.getLast().getValue().getDuration();
                    DVD dvdToAdd = new DVD(title, year, duration);
                    model.add(dvdToAdd);
                }else{
                    model.remove(redoList.getLast().getValue().getTitle());
                    update(model, -1);
                }
                update(model, -1);
            }
        ...
        addButton.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent actionEvent) {
              //Get DVD Information
              DVD  d = new DVD(tField.getText(), Integer.valueOf(yField.getText()),Integer.valueOf(lField.getText()));
              model.add(d);    // Add the DVD to the model
              performOperation(ADD_DVD, d);
              redoList.clear();
            ...
            public void handle(ActionEvent actionEvent) {
                if (tList.getSelectionModel().getSelectedItem() != null) {
                    ...
                    if (result.get() == ButtonType.OK){
                        performOperation(REMOVE_DVD, tList.getSelectionModel().getSelectedItem());
